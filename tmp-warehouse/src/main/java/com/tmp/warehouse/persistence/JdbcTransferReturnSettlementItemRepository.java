...
/** JDBC adapter for {@code warehouse.transfer_return_settlement_item}. */
@SuppressFBWarnings(
        value = "EI_EXPOSE_REP2",
        justification = "Stores Spring-managed JdbcTemplate.")
public final class JdbcTransferReturnSettlementItemRepository
        implements TransferReturnSettlementItemRepository {

    private final JdbcTemplate jdbc;

    public JdbcTransferReturnSettlementItemRepository(JdbcTemplate jdbc) {
        this.jdbc = Objects.requireNonNull(jdbc, "jdbc");
    }

    @Override
    public void insertAll(UUID documentId, List<TransferReturnSettlementItem> items) {
        Objects.requireNonNull(documentId, "documentId");
        Objects.requireNonNull(items, "items");
        for (TransferReturnSettlementItem item : items) {
            Objects.requireNonNull(item, "item");
            if (!documentId.equals(item.documentId())) {
                throw new IllegalArgumentException(
                        "Return item documentId mismatch: expected="
                                + documentId
                                + ", actual="
                                + item.documentId());
            }
            jdbc.update(
                    ...
                    item.returnStorageCellId().value(),
                    item.quantity().value(),
                    item.returnOperationId().value(),
                    Timestamp.from(item.createdAt()));
        }
    }

    @Override
    public List<TransferReturnSettlementItem> findByDocumentId(UUID documentId) {
        Objects.requireNonNull(documentId, "documentId");
        return List.copyOf(
                jdbc.query(
                        """
                        SELECT id, document_id, send_allocation_id, return_storage_cell_id,
                               quantity, return_operation_id, created_at
                        ...
                        (rs, rowNum) ->
                                TransferReturnSettlementItem.of(
                                        ...
                                        StorageCellId.of(
                                                (UUID) rs.getObject("return_storage_cell_id")),
                                        StockQuantity.of(rs.getBigDecimal("quantity")),
                                        WarehouseOperationId.of(
                                                (UUID) rs.getObject("return_operation_id")),
                                        rs.getTimestamp("created_at").toInstant()),
                        documentId));
    }

    @Override
    public List<TransferReturnSettlementItem> findBySendAllocationIds(
