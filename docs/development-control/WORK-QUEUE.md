...
- none in `com.tmp.security.api` yet (the public-facing `AuthenticationService` faзade is assembled in STAGE4-028 by delegating to this Application Service).
...
A fully-tested Authentication Application Service ready for the public faзade (STAGE4-028) and the Login Screen (STAGE4-032).
...
- none in `com.tmp.security.api` yet (public faзade assembled in STAGE4-028).
...
A fully-tested Authorization Application Service and a reusable secured-operation fixture, ready for the public faзade (STAGE4-028) and the Access Denied Screen / navigation-gating UI (STAGE4-033/034).
...
Fully-tested user administration, ready for password operations (STAGE4-024), the public faзade (STAGE4-028), and the User Administration Screen (STAGE4-035).
...
Fully-tested password operations, ready for the public faзade (STAGE4-028) and the User Administration Screen (STAGE4-035).
...
Fully-tested role administration, ready for the public faзade (STAGE4-028) and the Role Administration Screen (STAGE4-036).
...
Fully-tested assignment/override administration, ready for the public faзade (STAGE4-028) and the Role Administration Screen (STAGE4-036).
...
A fully-tested, read-only audit query service, ready for the public faзade (STAGE4-028) and the Security Audit Screen (STAGE4-037).
...
- defining more than one bean of any public faзade/service/`PasswordEncoder`/`SessionContext` type (enforced by simply not declaring duplicates ? verified by a Spring context test asserting exactly one bean per type, same style as `CapabilityEngineAutoConfigurationTest`);
...
- [ ] full Spring context (`tmp-bootstrap-app`-style test, or a focused `@SpringBootTest` within `tmp-security` using H2/Testcontainers) starts with exactly one bean of each public faзade/service type;
...
- `UserAdministrationViewModel`: observable list of `UserSummary` (loaded via `listUsers(...)`, paginated); `createUser(...)`, `updateUser(...)`, `deleteUser(...)`, `resetPassword(...)` methods delegating to `UserAdministrationService`/`PasswordApplicationService`-backed faзade methods (via `com.tmp.security.api` only), each wrapped to surface `AccessDeniedException`/validation failures as a bound error message (no stack trace).
...
- [ ] create/update/delete/reset actions call the correct faзade methods and refresh the list on success;
...
Stage 3.5.8.3 ? Reject + Return = COMPLETE (2026-09-09).
...
## STAGE-3.5.8.3 ? Reject + Physical Return + Final Settlement

**Status:** DONE
**Stage:** 3.5
**Depends on:** Stage 3.5.8.2
**Module:** tmp-warehouse (primary); additive public API; compile-only Production/UI test stubs

### Goal

Complete post-send physical settlement: whole-document reject (no continuation), TRANSFER_RETURN physical return, conservation close, RETURN_MATERIALS completion.
...
- [x] Explicit rejectTransferDocument + mandatory trimmed reason <=500
- [x] Destination responsibility + warehouse.transfer; rejectedBy from session
- [x] AWAITING_RECEIPT to RETURN_PENDING / REJECTED; no stock mutation; no continuation
- [x] V42 TRANSFER_RETURN CHECK + transfer_return_settlement_item
- [x] returnTransferMaterials default + exact override plan; source responsibility
- [x] Engine transferReturn (same-cell OK); settlement SETTLED preserving decision/metadata; document CLOSED
- [x] onClose accepted+returned=sent per sendAllocationId
- [x] Document-managed status corrective; receive/task Javadoc corrective
- [x] Concurrent reject/receive and return/return; rollback proofs
- [x] Targeted tests + architecture + quick build + package + runtime V42 smoke
- [x] No UI / no 3.5.9
