...
**Current Task:** none (Stage 3.5.10 complete; Warehouse SpotBugs VA_FORMAT fix applied; **`mvn verify` FAIL** on UI Shell SpotBugs — do not start 3.5.11)
...
**Active blockers:** Full reactor `mvn verify` FAIL (2026-09-10) — SpotBugs ×8 in `tmp-ui-shell` (`EI_EXPOSE_REP`/`EI_EXPOSE_REP2`/`BX_UNBOXING_IMMEDIATELY_REBOXED`). Warehouse `VA_FORMAT_STRING_USES_NEWLINE` ×6 corrected (PASS). Stage 3.5.11 blocked until verify GREEN. UI final manual acceptance still pending (not a code blocker).
...
**Full verify baseline:** RED — Warehouse SpotBugs fixed; verify stops at UI Shell SpotBugs (2026-09-10). Full GREEN baseline not established
**First READY implementation task:** SpotBugs fix in `tmp-ui-shell` (CustomerFilterKey/ItemProductionStateReader/OrderOperationalListResult/OrderImportViewModel) before Stage 3.5.11; then Stage 3.5.11 Modern Остатки (NOT STARTED)
