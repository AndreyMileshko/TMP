...
**Current Task:** none (Stage 3.5.10 complete; OM Flyway bootstrap IT corrective applied; **`mvn verify` FAIL** on Warehouse SpotBugs — do not start 3.5.11)
...
**Active blockers:** Full reactor `mvn verify` FAIL (2026-09-10) — SpotBugs `VA_FORMAT_STRING_USES_NEWLINE` ×6 in `tmp-warehouse` JDBC persistence (SQL `.formatted()` with `\\n`). Prior OM `OrderIntakeFlywayBootstrapIT` head 25→34 corrective PASS. Stage 3.5.11 blocked until verify GREEN. UI final manual acceptance still pending (not a code blocker).
...
**Full verify baseline:** RED — OM bootstrap fixed; verify stops at Warehouse SpotBugs (2026-09-10). Full GREEN baseline not established
**First READY implementation task:** SpotBugs `VA_FORMAT_STRING_USES_NEWLINE` fix/suppress in warehouse JDBC batch SQL (before Stage 3.5.11); then Stage 3.5.11 Modern Остатки (NOT STARTED)
