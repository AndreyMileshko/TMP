...
## Stage 5 Documentation Gate — final corrections (STAGE5-000-FIX2)

Documentation Gate был повторно открыт для финальных исправлений (на время работ `STAGE5-001` → `PLANNED`, `STAGE5-000-FIX2` → единственная `READY`). Выявленные документационные противоречия и их устранение:

| # | Замечание | Устранение | Статус |
|---|---|---|---|
| DOC-BLK-7 | After-commit публикация опиралась на внутренний класс Document Engine (`TransactionAfterCommitEventPublisher`); Order Management импортировал бы внутреннюю реализацию. | Введён публичный контракт `TransactionalEventPublisher { void publishAfterCommit(DomainEvent); }` (Document Engine Specification v1.1 §«Публичный after-commit механизм»); OM Spec §12/§17/§21, Manifest §11, CONTEXT-MAP переведены на публичный контракт; добавлена prerequisite-задача `STAGE5-017` (контракт + Spring transaction synchronization adapter + тесты commit/rollback) до первого Document Processor; architecture test `STAGE5-047` запрещает импорт внутренних классов Document Engine. | CLOSED |
| DOC-BLK-8 | Физическая модель typed payload не была определена (риск JSON/сериализации). | OM Spec §11.5/§19: `order_document_payload` (общие metadata, ключ `document_id`) + typed-таблицы (`order_create_payload`, `order_update_payload`, `order_status_payload`, `order_item_create_payload`, `order_item_update_payload`, `order_item_status_payload`, `order_item_revision_create_payload`, `order_item_revision_update_payload`, `order_item_revision_approve_payload`) + `order_item_revision_payload_line`; связь через `document_id` (FK), optimistic lock `payload_revision`, каскадное удаление Draft, immutability после проведения, коллекции — отдельными строками, JSON/сериализация запрещены; задачи `STAGE5-016`/`STAGE5-020`/`STAGE5-043` обновлены. | CLOSED |
| DOC-BLK-9 | Idempotency описывалась как «повторный `onPost` возвращает сохранённый результат». | OM Spec §14.1/§16 и Manifest §10: публичный повторный `DocumentEngine.postDocument` проведённого документа отклоняется lifecycle validation; guard внутри processor при существующей processing record завершается как already processed (без повторного изменения/события/записи), `resultReference` внутренний, `DocumentProcessor.onPost()` возвращает `void`; задачи `STAGE5-018`/`STAGE5-045` обновлены; упоминание возврата результата удалено. | CLOSED |
| DOC-BLK-10 | Транзакционный контракт Document Engine не был зафиксирован в его спецификации; номера/версии рассинхронизированы (`STAGE5-001..045`, GUI smoke `045`, CONTEXT-MAP «v1.1»). | Document Engine Specification → v1.1 (транзакционный контракт lifecycle-операций + публичный after-commit); синхронизированы номера (`STAGE5-001..050`, GUI smoke `STAGE5-050`) в Manifest; CONTEXT-MAP «Order Management Specification v1.2»; очередь пересобрана (50 задач) с корректным порядком §8. | CLOSED |

Все замечания STAGE5-000-FIX2 — **CLOSED**. Публичный `TransactionalEventPublisher` реализуется как задача очереди `STAGE5-017` (prerequisite), а не как блокер. Stage 5 не заблокирован; `STAGE5-001` возвращён в `READY`. Active blocker: None.
