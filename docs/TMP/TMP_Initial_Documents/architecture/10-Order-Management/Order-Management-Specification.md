...
* приём заказа через ручной ввод и через файловый импорт выгрузки расчётной программы (MVP);
* source-neutral граница импорта без зависимости домена от формата файла или внешней СУБД;
...
| External Position Number | Внешний номер расчётной позиции (`externalPositionNumber`); не UUID |
| Код изделия | Код изделия (`ProductCode`); **nullable только в `DRAFT`** (ADR-030 / STAGE5-052A) |
| Наименование | Наименование изделия; **nullable только в `DRAFT`** |
...
**Incomplete DRAFT (позиция):** в статусе `DRAFT` допускается временное отсутствие `productCode` и/или `name` (`null`). Запрещены placeholders (`UNKNOWN`, `N/A`, `IMPORT`, `—`, «Изделие N» и т.п.). `externalPositionNumber` **не** заменяет `productCode`. `materialName` строки спецификации **не** заменяет наименование Order Item. Перед `ORDER_ITEM_REVISION_APPROVE` обязательны заполненные `productCode` и `name`, непустая спецификация и прочие инварианты утверждения. Позиция в статусе `ACTIVE` всегда имеет `productCode` и `name`. Позиция в статусе `CANCELLED` может хранить последнее состояние ранее неполного `DRAFT` (включая `null` commercial fields); это не разрешает редактировать коммерческие поля в `CANCELLED`.
...
### Атрибуты строки спецификации (MVP)

| Атрибут | Назначение | Правила |
| --- | --- | --- |
| `materialCode` | Внешний артикул / код материала | non-blank; не внутренний UUID |
| `materialName` | Наименование материала | non-blank; может содержать символ `/` |
| `color` | Цвет как производственный снимок | nullable, если цвет неприменим; одинаковый артикул с разным цветом — разные потребности |
| `lengthMm` | Длина одной детали, мм | `null` = неприменимо; если задано — `> 0`; `<= 0` — ошибка; не хранить строку с суффиксом «мм» |
| `lineQuantity` | Общее число одинаковых элементов строки для расчётной позиции | `!= null`, `> 0`; **запрещено** умножать на `productQuantity` повторно |
| `unitOfMeasure` | Единица измерения количества строки | обязательна в домене; для STXT/ручного MVP по умолчанию «шт», пока нет справочника материалов |

**`consumptionNorm`:** в подтверждённом процессе MVP отдельного бизнес-смысла нет. Не использовать как замену `lengthMm`, `lineQuantity` или `productQuantity`. Планируется безопасное удаление/замена в рамках STAGE5-051 (см. анализ текущей реализации). Warehouse / Production / Cutting Optimization на этот контракт не опираются.

**Ориентация / расположение:** исключены из модели. Не добавлять `orientation`, `placement`, `positionDirection`, `VERTICAL`, `HORIZONTAL`.
...
Перенесены за пределы текущего MVP Order Intake / Stage 5 core:
...
* merge / автосравнение / перезапись при повторном импорте существующего заказа;
...
- **AR-013** Файловый импорт и ручной ввод создают одинаковую доменную структуру через общие application services (ADR-029).
...
# 27. Order Intake MVP (файловый импорт и ручной ввод)

## 27.1 Каналы поступления

В MVP два способа создания заказа:

1. **Файловый импорт** выгрузки из расчётной программы («СуперОкна», формат `.stxt` / текстовая выгрузка).
2. **Ручной ввод** заказа, позиций и спецификаций в UI.

Оба способа формируют одну и ту же структуру:

```text
Order → Order Item → Item Revision → Specification Lines
```

Обязательно общие: доменные правила, application services, бизнес-документы, статусы, разрешения, правила редакций/утверждения, persistence contracts.

## 27.2 Firebird / БД «СуперОкна»

Прямое подключение к Firebird **вне MVP**. Не реализовывать JDBC Firebird, SQL к таблицам расчётной программы, фоновый опрос, расписания, двусторонний обмен, plugin framework, отдельный Maven-модуль только ради Firebird.

Архитектурная граница (ADR-029):

```text
External source → source adapter → source-neutral import model → Order Management application services
```
...
→ одна транзакция:
   DRAFT Order + Items + DRAFT Revisions + Specification lines
```
...
5. Фиктивные значения запрещены: `UNKNOWN`, `N/A`, `IMPORT`, `—`, «Изделие N», пустые системные заглушки.
6. Пользователь дополняет коммерческие данные вручную.
7. `ORDER_APPROVE` запрещён, пока обязательные коммерческие поля заказа не заполнены; ошибка утверждения должна перечислять отсутствующие обязательные поля.
8. `ORDER_ITEM_REVISION_APPROVE` запрещён, пока у позиции не заполнены `productCode` и `name` (и непустая спецификация); ошибка перечисляет «Код изделия» / «Наименование изделия».
9. После утверждения применяются обычные правила неизменяемости; утверждённая позиция всегда имеет `productCode` и `name`. `CANCELLED` может хранить последнее состояние ранее неполного `DRAFT`; редактирование коммерческих полей в `CANCELLED` запрещено.
10. Правило доменное для `DRAFT`, независимо от STXT.

**Реализация:** order-level incompleteness — `STAGE5-051`; item-level incompleteness — `STAGE5-052A` (до Import Core).

## 27.8 Ручной ввод

Полноценный канал MVP. UI должен поддерживать:
...
Ручной ввод и импорт создают одинаковую доменную структуру.

## 27.9 Анализ текущей реализации (документационный baseline)

| Область | Сейчас | Gap для MVP |
| --- | --- | --- |
| Order Item | `productCode`, `name`, `comments`; нет `externalPositionNumber` | добавить `externalPositionNumber` |
| productQuantity | `OrderedQuantity` на Revision | сохранить как единый источник; UI-лейбл «Количество изделий» |
